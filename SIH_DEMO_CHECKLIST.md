# SIH 2026 JUDGING DEMO CHECKLIST & OPERATING GUIDE
...
**Core Flow:** `DOCUMENT → IDENTITY → EVIDENCE → NETWORK → TIMELINE → INTELLIGENCE → SECURITY`

---

## BEFORE DEMO (Technical Pre-Flight Checks)

- [ ] **1. Start Server:** Run `npm run dev` or `npm run build && npm start` in `SIH-Hackathon2.2` on port 3000.
...
- [ ] **5. Confirm Primary Case Available:** Open `/cases/C-001` (Operation Trishul) -> Verify Case Workspace link & dynamic degree.
- [ ] **6. Confirm Document Forensics:** Open `/documents/D-001` -> Verify OCR (96%), MRZ, and Forensics (82% Suspicious).
- [ ] **7. Confirm Knowledge Graph:** Open `/network` -> Verify Rahul Mehra (P-1042) centered at hub with dynamic degree 7.
...
- [ ] **11. Confirm AI Insights & Brief:** Open `/insights` -> Open "Why This Insight?" modal; open Case Workspace -> Generate Section 65B-oriented Brief.
- [ ] **12. Confirm Append-Only Audit Ledger:** Open `/audit` -> Verify runtime recorded actions appear in real-time.
...
| Time | Stage | Route | Key Talking Points for Judges |
| :---: | :--- | :--- | :--- |
| **0:00** | **1. Secure Access & Session** | `/login` | Click **"⚡ ONE-CLICK DEMO ACCESS"**. Explain role separation, strict credential validation, cryptographic HMAC-SHA256 session tokens, and zero token leakage to client JS. |
| **0:25** | **2. Command Dashboard** | `/dashboard` | Highlight 6-stage investigation pipeline, live intelligence counts, and the **Primary Demo Focus (Case #2026-041 — Operation Trishul)**. |
| **0:45** | **3. Unified Case Workspace** | `/cases/C-001/workspace` | Open Case Workspace. Highlight Change Monitor ("What Changed?"), quick action bar, Rahul Mehra primary subject badge (degree 7), and investigation health score. |
| **1:10** | **4. Document Forensics** | `/documents/D-001` | Inspect evidentiary identity card **DOC-2026-041-009**. Show automated OCR (96%), font irregularity detection, and photo boundary anomaly. |
...
- [ ] **2. Confirm Clean State:** Dashboard displays clean confirmation banner and all demo state is deterministically restored.
